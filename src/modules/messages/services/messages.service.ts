...
export class MessagesService {
  ...
  async sendMessage(userId: string, conversationId: string, dto: CreateMessageDto) {
    ...
    if (dto.messageType !== 'TEXT' && (!dto.mediaIds || dto.mediaIds.length === 0)) {
      throw new BadRequestException('Media message requires at least one media attached');
    }
    ...
    const message = await this.prisma.$transaction(async (tx) => {
      const newMsg = await tx.message.create({
        ...
        select: {
          id: true, content: true, messageType: true, status: true, createdAt: true,
          senderUserId: true,
          sender: { select: { username: true, profile: { select: { displayName: true, avatarMediaId: true } } } }
        }
      });
  ...
  async getMessages(userId: string, conversationId: string, cursor?: string, limit = 50) {
    ...
    const messages = await this.prisma.message.findMany({
      ...
      select: {
        ...
        sender: { select: { username: true, profile: { select: { displayName: true, avatarMediaId: true } } } },
        attachments: {
          select: { mediaAsset: { select: { id: true, objectKey: true, bucket: true, mimeType: true } } }
        }
      },
      take,
  ...
  async editMessage(userId: string, messageId: string, content: string) {
    ...
    if (message.senderUserId !== userId) throw new ForbiddenException('Cannot edit others message');
    if (message.status === MessageStatus.DELETED) throw new BadRequestException('Cannot edit deleted message');
    ...
    this.messagesGateway.broadcastMessageUpdate(message.conversationId, messageId, content);
    return updated;
  }
  ...
  async deleteMessage(userId: string, messageId: string) {
    const message = await this.prisma.message.findUnique({ where: { id: messageId } });
    if (!message) throw new NotFoundException('Message not found');
    if (message.senderUserId !== userId) throw new ForbiddenException('Cannot delete others message');
    ...
    this.messagesGateway.broadcastMessageDelete(message.conversationId, messageId);
